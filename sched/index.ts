...
const debug = Debug('sched:verbose');
const debugInfo = Debug('sched');
...
export class Sched extends EventEmitter {
    ...
    async loop(repeat = false) {
        ...
        try {
            ...
            debugInfo('problem in sched:', err);
    ...
    reset_loop(tick: number) {
        ...
        if(tick)
            ...
            this.loop(true).then(_.noop, _.noop);
    }
    ...
    async init() {
        ...
        for (const job of this.get_scheduled_jobs()) {
            ...
            runAndSchedule().then(_.noop, debugInfo);
        }
    }
...
if(module === require.main) {
    load_db(load_config().db).then(async (db) => {
